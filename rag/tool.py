"""
RAG tools — store and retrieve documents from ChromaDB collections.
...
def rag_store(collection_name: str, document: str, metadata: str) -> str:
    ...
    # Ensure all metadata values are str, int, float, or bool (ChromaDB requirement)
    clean_meta = {}
    ...
    logger.info(
        "rag_stored collection=%s chunks=%d total_docs=%d",
        collection_name, len(chunks), collection.count(),
    )
    return f"Successfully stored {len(chunks)} chunk(s) in collection '{collection_name}'."
...
def rag_retrieve(collection_name: str, query: str, n_results: int = 5) -> str:
    ...
    for i, (doc, dist) in enumerate(zip(documents, distances), 1):
        ...
        lines.append(f"{i}. [similarity={similarity:.3f}] {doc}")

    logger.info(
        "rag_retrieved collection=%s query_len=%d results=%d",
        collection_name, len(query), len(documents),
    )
    return "\n".join(lines)
